...
var Utils = function () {
    ...
    _createClass(Utils, null, [{
        key: "copyMatrix",
        value: function copyMatrix(array) {
            var n = [];
        ...
        key: "shapeToCoords",
        value: function shapeToCoords(shape, xOffset, yOffset) {
            var coords = [];
            var x = void 0;
            var y = void 0;
        ...
        key: "selectRandom",
        value: function selectRandom(array) {
            return array[Math.floor(Math.random() * array.length)];
        }
    }, {
        key: "matrixToCoords",
        value: function matrixToCoords(matrix) {
            var coords = [];
...
var windowConstants = {
    ...
    var game = new _Game2.default(settings, function (game) {
        game.run();
    });
}
// run on window load
(0, _jquery2.default)(document).ready(run);
...
var Game = function () {
    function Game(settings, callback) {
    ...
    _createClass(Game, [{
        ...
        value: function init(settings, callback) {
            ...
            this.loadPieces(function () {
                ...
                for (var _i = 0; _i < settings.grid.width; _i++) {
        ...
        key: "checkRotationCollision",
        value: function checkRotationCollision(piece) {
            // theoreticallys rotate the piece, and add that to the original piece creating 1 new piece.
            var coords = piece.getCoords();
            var rotated = _Utils2.default.shapeToCoords(_Utils2.default.matrixToCoords(piece.states[(piece.state + 1) % piece.states.length]), piece.pos.x, piece.pos.y);
            ...
            var lowestX = Math.min.apply(null, xValues);
            var highestX = Math.max.apply(null, xValues);
            var lowestY = Math.min.apply(null, yValues);
            var highestY = Math.max.apply(null, yValues);
            // avoid self collisions
            var tempGrid = _Utils2.default.copyMatrix(this.grid);

            for (var _i2 = 0; _i2 < piece.shape.length; _i2++) {
                var x = piece.shape[_i2][0] + piece.pos.x;
                var y = piece.shape[_i2][1] + piece.pos.y;
                if (x > -1 && y > -1) {
                    tempGrid[x][y] = null;
                }
            }
            if (lowestX < 0 || highestX > this.constants.gridWidth - 1 || lowestY < 0 || highestY > this.constants.gridHeight - 1) {
                return true;
            }
            // check illegal positions
            for (var _x = lowestX; _x < highestX + 1; _x++) {
                for (var _y = lowestY; _y < highestY + 1; _y++) {
                    if (tempGrid[_x][_y] != null) {
                        return true;
                    }
                }
            }
        ...
        value: function clearLines() {
            var lines = Array(this.grid[0].length);
            ...
            var empty = 0;
            for (var _y2 = lines.length - 1; _y2 > 0; _y2--) {
                if (lines[_y2]) {
                    for (var _x2 = 0; _x2 < this.grid.length; _x2++) {
                        this.grid[_x2].splice(_y2, 1);
                    }
                    empty++;
                }
            }
            for (var _x3 = 0; _x3 < this.grid.length; _x3++) {
                for (var _i3 = 0; _i3 < empty; _i3++) {
                    this.grid[_x3].splice(0, 0, null);
                }
            }
        }
        /**
        ...
        key: "screenInit",
        value: function screenInit(settings) {
        ...
        key: "loadPieces",
        value: function loadPieces(callback) {
            var game = this;
        ...
        value: function generateRandomPiece() {
            return new _Piece2.default(this, _Utils2.default.selectRandom(Object.keys(this.constants.pieces)));
        }
        /**
        ...
        value: function newPiece() {
            var p = this.generateRandomPiece();
            p.pos.y = -1;
        ...
        key: "writePieceToGrid",
        value: function writePieceToGrid(piece) {
            for (var i = 0; i < piece.shape.length; i++) {
                var x = piece.shape[i][0] + piece.pos.x;
                var y = piece.shape[i][1] + piece.pos.y;
                if (x > -1 && y > -1) {
        ...
        key: "clearPieceFromGrid",
        value: function clearPieceFromGrid(piece) {
            for (var i = 0; i < piece.shape.length; i++) {
                var x = piece.shape[i][0] + piece.pos.x;
                var y = piece.shape[i][1] + piece.pos.y;
                if (x > -1 && y > -1) {
        ...
        key: "floorCheck",
        value: function floorCheck(piece) {
            ...
            for (var _i4 = 0; _i4 < piece.shape.length; _i4++) {
                var _x4 = piece.shape[_i4][0] + piece.pos.x;
                var _y3 = piece.shape[_i4][1] + piece.pos.y;
                if (_y3 >= this.constants.gridHeight - 1 || tempGrid[_x4][_y3 + 1] != null) {
                    return true;
                }
            }
            return false;
        ...
        key: "checkPieceCollision",
        value: function checkPieceCollision(piece, offsetX, offsetY) {
            ...
            for (var _i5 = 0; _i5 < piece.shape.length; _i5++) {
                var _x5 = piece.shape[_i5][0] + offsetX + piece.pos.x;
                var _y4 = piece.shape[_i5][1] + offsetY + piece.pos.y;
                if (_y4 > this.constants.gridHeight - 1 || _x5 < 0 || _x5 > this.constants.gridWidth - 1 || tempGrid[_x5][_y4] != null) {
                    return true;
                }
            }
            return false;
        ...
        key: "movePieceBy",
        value: function movePieceBy(piece, x, y) {
        ...
        key: "progressPiece",
        value: function progressPiece(piece) {
            if (!this.floorCheck(piece)) {
        ...
        value: function renderPiece(piece) {
            ...
            for (var i = 0; i < piece.shape.length; i++) {
                ...
                var y = piece.shape[i][1];
                this.screen.fillRect((piece.pos.x + x) * this.constants.blockSize, (piece.pos.y + y) * this.constants.blockSize, this.constants.blockSize, this.constants.blockSize);
            }
            this.screen.restore();
        }
        /**
            ...
            for (var x = 0; x < this.grid.length; x++) {
                for (var y = 0; y < this.grid[x].length; y++) {
                    ...
                    if (current != null && current != undefined) {
                        ...
                        this.screen.fillRect(x * this.constants.blockSize, y * this.constants.blockSize, this.constants.blockSize, this.constants.blockSize);
                    }
                }
            }
            this.screen.restore();
...
var Piece = function () {
    ...
    _createClass(Piece, [{
        ...
        value: function getCoords() {
            return _Utils2.default.shapeToCoords(this.shape, this.pos.x, this.pos.y);
        }
        /**
        ...
        value: function update() {
            ...
            this.shape = _Utils2.default.matrixToCoords(this.grid);
        }
    }, {
        key: "moveBy",
